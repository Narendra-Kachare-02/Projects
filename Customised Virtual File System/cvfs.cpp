...
#include<string.h>      // for some string inbuilt function
#include<unistd.h>      // System calls
#include<iostream>      //
//#include<io.h>            g++ cvfs.cpp -o Myexe -save-temps   .i file will c reate

#define MAXINODE 50     // evdya file kru shakto (50) (per file ek INODE)
...
void man(char *name)                                // 2.5  (command[1])   (11)
{
    if(name == NULL)return;
...
int main()
{
    ...
    char*ptr = NULL;    // used for memory allocation
    int ret = 0, fd = 0, count = 0;
    char command[4][80],str[80], arr[1024];     // local variables

    InitialiseSuperBlock();     //Auxilary data initialisation  (Before application execute application should set all necessities)
    CreateDILB();

    while(1)        //lect chalu zalyaverchi porcess
    {
        fflush(stdin);  // pudhchi command takal teva  magchya command che kay rahile asel tr flush houn jael.  (scanf madhe rahilele space)
        strcpy(str,""); // str navachi string clear keli
        printf("\nMarvellous VFS : > ");    // shell header

        fgets(str,80,stdin);// scanf("%[^'\n]s" ,str);

        count = sscanf(str,"%s %s %s %s",command[0],command[1],command[2],command[3]);   // string scanf madhe ghya input space dila ki string end
